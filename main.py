...
def merge_csv(to_path, from_path=None, directory=None):
    setup(to_path)
    if directory:
        if type(directory) == str:
            for path in glob.iglob(r'C:%s\*.csv' % (directory), recursive=True):
                dataframe = pd.read_csv(path, index_col=0)
                dataframe.to_csv(to_path, mode='a', header=False, index=True)
        elif type(directory) == list:
            for folder_path in directory:
                for path in glob.iglob(r'C:%s\*.csv' % (folder_path), recursive=True):
                    dataframe = pd.read_csv(path, index_col=0)
                    dataframe.to_csv(to_path, mode='a', header=False, index=True)
    if from_path:
        if type(from_path) == str:
            dataframe = pd.read_csv(from_path, index_col=0)
            dataframe.to_csv(to_path, mode='a', header=False, index=True)
        elif type(from_path) == list:
...
if __name__ == "__main__":
    ...
    #                 writer = csv.writer(f)
    #                 writer.writerow(["Last Logged Time: " + str(current_time)])
